...
快照允许可以一个需要备份的时间点以防某个文件被删除或被覆盖。比如，我们创建一个快照，当在用户主目录下删除一些目录后，然后把他恢复。
...
这个仅仅是一个扼要的 Ubuntu下 ZFS 的用法。更多的信息请参考 [Ubuntu’s wiki page on ZFS.][5] 我也推荐阅读 [ArsTechnica的精彩文章][8]。

这个是实验性的功能，如果你还不了解 ZFS。如果你想用一个简单稳定的系统，请安装标准文件系统 EXT4。如果你想用闲置的机器体验，可以参照上面了解 ZFS。如果你是一个‘专家’，你知道你做的事情，那就可以随便咋搞。

你之前用过 ZFS吗？请在下面留言。如果你觉得这个文章还可以，请分享到社交媒体，黑客新闻或[Reddit][9]。
